...
function replacePrefix {
    ...
    sed -i "s/%BD_SUITE_PREFIX%/${BD_HDFS_PREFIX}/g" ${DRUID_HOME}/conf/druid/_common/hdfs-site.xml
    sed -i "s/%BD_JOURNALNODE_SERVERS%/${BD_JOURNALNODE_SERVERS}/g" ${DRUID_HOME}/conf/druid/_common/hdfs-site.xml
    sed -i "s/%BD_SUITE_PREFIX%/${BD_HDFS_PREFIX}/g" ${DRUID_HOME}/conf/druid/_common/yarn-site.xml
    sed -i "s/%BD_ZOOKEEPER_SERVERS%/${BD_ZOOKEEPER_SERVERS}/g" ${DRUID_HOME}/conf/druid/_common/yarn-site.xml
    sed -i "s/%BD_SUITE_PREFIX%/${BD_HDFS_PREFIX}/g" ${DRUID_HOME}/conf/druid/_common/mapred-site.xml
    sed -i "s/%BD_SUITE_PREFIX%/${BD_SUITE_PREFIX}/g" ${DRUID_HOME}/conf/druid/_common/common.runtime.properties
    sed -i "s/%BD_MYSQL_PREFIX%/${BD_MYSQL_PREFIX}/g" ${DRUID_HOME}/conf/druid/_common/common.runtime.properties
    ...
    if [ ${BD_COMPONENT_OR_AMAH} == "component" ]; then
        sed -i "s/%DB_DRUID_EMITTER%/logging/g" ${DRUID_HOME}/conf/druid/_common/common.runtime.properties
    else
        sed -i "s/%DB_DRUID_EMITTER%/http/g" ${DRUID_HOME}/conf/druid/_common/common.runtime.properties
        sed -i "s/%BD_AMAH_SERVER%/${BD_AMAH_SERVER}/g" ${DRUID_HOME}/conf/druid/_common/common.runtime.properties
    fi
}
