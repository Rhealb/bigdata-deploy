...
function replacePrefix {
    sed -i "s/%BD_SUITE_PREFIX%/${BD_HADOOP_PREFIX}/g" ${SPARK_HOME}/conf/core-site.xml
    sed -i "s/%BD_ZOOKEEPER_SERVERS%/${BD_ZOOKEEPER_SERVERS}/g" ${SPARK_HOME}/conf/core-site.xml

    sed -i "s/%BD_SUITE_PREFIX%/${BD_HADOOP_PREFIX}/g" ${SPARK_HOME}/conf/hdfs-site.xml
    sed -i "s/%BD_JOURNALNODE_SERVERS%/${BD_JOURNALNODE_SERVERS}/g" ${SPARK_HOME}/conf/hdfs-site.xml

    sed -i "s/%BD_SUITE_PREFIX%/${BD_HADOOP_PREFIX}/g" ${SPARK_HOME}/conf/yarn-site.xml
    ...
    sed -i "s|%BD_SUITE_PREFIX%|${BD_SUITE_PREFIX}|g" ${SPARK_HOME}/conf/spark-env.sh
