{
  ...
  mysql: {
    ...
    externalports: {
     ...
     nodeports: {
      mysqlports: ["" for count in std.range(1, $.mysql.instancecount)],
    },
  ...
  clickhouse: { 
    image: $.registry + "/clickhouse/clickhouse-server:1.1.54380-isLocal-1.0",
    exservicetype: $.exservicetype, 
    zkprefix: $.suiteprefix,
    zkinstancecount: $.zookeeper.instancecount,
    instancecount: 3, 
    ...
    limitmem: "0", 
    externalports: { 
      port1s:[8123 + i for i in std.range(0,$.clickhouse.instancecount - 1)],
      port2s:[9000 + i for i in std.range(0,$.clickhouse.instancecount - 1)],
      port3s:[9009 + i for i in std.range(0,$.clickhouse.instancecount - 1)],
       
    }, 
    nodeports: { 
      port1s:["" for count in std.range(1, $.clickhouse.instancecount)],
      port2s:["" for count in std.range(1, $.clickhouse.instancecount)],
      port3s:["" for count in std.range(1, $.clickhouse.instancecount)],
       
    }, 
    componentoramah: $.componentoramah,
    amah: { 
      image: $.registry + "/tools/amah-clickhouse-1.1.54380-islocal-1.0:0.0.3-snapshot", 
      exservicetype: $.exservicetype, 
      clickhouseinstancecount: $.clickhouse.instancecount,
      instancecount: 1, 
      replicas: 1, 
      requestcpu: "0", 
      requestmem: "0", 
